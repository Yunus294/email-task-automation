...
describe("email -> task flow (e2e)", () => {
  ...
  describe("webhook", () => {
    ...
    it("drops assignees that are not users of the company", async () => {
      ...
      expect(task.assigneeEmail).toBeNull();
  ...
  describe("tasks api", () => {
    ...
    it("filters by status and paginates", async () => {
      const res = await getTasks(ACME_KEY, "?status=PENDING_REVIEW&page=1&size=1").expect(200);
    ...
    it("does not allow reviewing twice", async () => {
      ...
      expect(res.body.code).toBe("TASK_ALREADY_REVIEWED");
    });
