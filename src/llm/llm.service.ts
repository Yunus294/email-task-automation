...
export class LlmService {
  ...
  async analyzeEmail(email: EmailForAnalysis): Promise<EmailAnalysis> {
    ...
    if (!apiKey) {
      throw new Error("OPENAI_API_KEY is not set (or use LLM_FAKE=true for local testing)");
    }
    ...
    if (!message || message.refusal || !message.content) {
      this.logger.warn(`Model returned no usable content (refusal: ${message?.refusal ?? "-"})`);
  ...
  private toAnalysis(raw: any): EmailAnalysis {
    ...
    const title = typeof raw?.title === "string" ? raw.title.trim().slice(0, MAX_TITLE_CHARS) : null;
  ...
  private fakeAnalyze(email: EmailForAnalysis): EmailAnalysis {
    this.logger.warn("LLM_FAKE is on, using keyword heuristic instead of OpenAI");
    const text = `${email.subject}\n${email.body}`;
    const isActionable = /(please|can you|could you|need|deadline|due|urgent|asap|fix|invoice)/i.test(
      text,
    );
    ...
    return {
      isActionable: true,
      title: (email.subject || email.body.split("\n")[0] || "Task from email").slice(0, MAX_TITLE_CHARS),
